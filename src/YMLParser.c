...
static bool is_hex(const char *s, size_t len) {
    if (len < 3 || s[0] != '0' || s[1] != 'x') return false;
...
static bool is_octal(const char *s, size_t len) {
    if (len < 3 || s[0] != '0' || s[1] != 'o') return false;
    for (size_t i = 2; i < len; i++)
...
static YMLValue infer_scalar(const char *s, size_t len) {
    ...
    if ((len==4 && memcmp(s,".nan",4)==0) || (len==4 && memcmp(s,".NaN",4)==0) ||
        ...
        v.type = YML_FLOAT; v.value.number = NAN; return v;
    }
    ...
    if (is_decimal(s, len)) {
        ...
        v.type = YML_INT; v.value.integer = (int64_t)strtoll(tmp, NULL, 10); return v;
    }
    if (is_hex(s, len)) {
        char tmp[64]; size_t n = len < 63 ? len : 63;
        memcpy(tmp, s, n); tmp[n] = '\0';
        v.type = YML_INT; v.value.integer = (int64_t)strtoll(tmp+2, NULL, 16); return v;
    }
    if (is_octal(s, len)) {
        char tmp[64]; size_t n = len < 63 ? len : 63;
        memcpy(tmp, s, n); tmp[n] = '\0';
        v.type = YML_INT; v.value.integer = (int64_t)strtoll(tmp+2, NULL, 8); return v;
    }
    ...
    /* float */
    {
        ...
        for (size_t i = 0; i < rem; i++) {
            if (p2[i]=='.' || p2[i]=='e' || p2[i]=='E') { has_dot_or_e=true; break; }
        }
        if (has_dot_or_e) {
            char tmp[64]; size_t n = len < 63 ? len : 63;
            memcpy(tmp, s, n); tmp[n] = '\0';
            ...
            if (end == tmp + n) {
                v.type = YML_FLOAT; v.value.number = d; return v;
            }
        }
    }
...
static char *unescape_double_quoted(const char *src, size_t len) {
    ...
    for (size_t i = 0; i < len; ) {
        ...
        case 't': case '\t': buf[w++] = '\t'; break;
        case 'n':  buf[w++] = '\n'; break;
        case 'v':  buf[w++] = '\v'; break;
        case 'f':  buf[w++] = '\f'; break;
        case 'r':  buf[w++] = '\r'; break;
        case 'e':  buf[w++] = 0x1B; break;
        case '"':  buf[w++] = '"';  break;
        case '/':  buf[w++] = '/';  break;
        case '\\': buf[w++] = '\\'; break;
        case 'x':  /* \xXX */
            if (i+2 <= len) { buf[w++]=(char)parse_hex_n(src+i,2); i+=2; }
            break;
        case 'u':  /* \uXXXX */
            if (i+4 <= len) { w+=(size_t)encode_utf8(parse_hex_n(src+i,4), buf+w); i+=4; }
            break;
        case 'U':  /* \UXXXXXXXX */
            if (i+8 <= len) { w+=(size_t)encode_utf8(parse_hex_n(src+i,8), buf+w); i+=8; }
            break;
        case '\n': /* line continuation: skip leading whitespace on next line */
            while (i < len && (src[i]==' '||src[i]=='\t'||src[i]=='\n'||src[i]=='\r')) i++;
            break;
        default: buf[w++] = esc; break;
        }
    }
    buf[w] = '\0';
    return buf;
...
static char *unescape_single_quoted(const char *src, size_t len) {
    ...
    size_t w = 0;
    for (size_t i = 0; i < len; ) {
        if (src[i]=='\'' && i+1<len && src[i+1]=='\'') { buf[w++]='\''; i+=2; }
        else buf[w++] = src[i++];
    }
    buf[w] = '\0';
    return buf;
...
static YMLValue *yml_deep_copy(const YMLValue *src) {
    ...
    case YML_OBJECT: {
        ...
        size_t idx = 0; const char *key; YMLValue *val;
        while (hm_next(src_hm, &idx, &key, &val)) {
            YMLValue *cp = yml_deep_copy(val);
            if (cp) { hm_set(dst_hm, key, *cp); free(cp); }
        }
...
static YMLValue *parse_scalar(Parser *p) {
    Token *t = cur(p);
    if (t->type != TK_SCALAR) { parse_error(p, "expected scalar"); return NULL; }
    advance(p);

    YMLValue *v = malloc(sizeof(YMLValue));
    if (!v) { parse_error(p, "OOM"); return NULL; }
...
static YMLValue *parse_flow_sequence(Parser *p) {
    ...
    while (cur(p)->type != TK_FLOW_SEQ_END && cur(p)->type != TK_EOF) {
        if (cur(p)->type == TK_FLOW_ENTRY) { advance(p); continue; }
        YMLValue *elem = parse_node(p, 0);
        if (p->ok || !elem) break;
        da_push(v->value.array, *elem);
        free(elem);
    }
    if (cur(p)->type == TK_FLOW_SEQ_END) advance(p);
    return v;
}
...
static YMLValue *parse_flow_mapping(Parser *p) {
    ...
    _hm *hm = hm_new(8);
    YMLValue *v = malloc(sizeof(YMLValue));
    if (!v || !hm) { parse_error(p, "OOM"); return NULL; }
    v->type = YML_OBJECT;
    v->value.object = hm;

    while (cur(p)->type != TK_FLOW_MAP_END && cur(p)->type != TK_EOF) {
        if (cur(p)->type == TK_FLOW_ENTRY) { advance(p); continue; }
        if (cur(p)->type == TK_MAP_KEY)    { advance(p); } /* explicit ? */

        /* ключ должен быть скаляром */
        if (cur(p)->type != TK_SCALAR) { parse_error(p, "expected scalar key in flow mapping"); break; }
        char *key = token_strdup(cur(p));
        advance(p);

        if (cur(p)->type != TK_MAP_VAL) { free(key); parse_error(p, "expected ':' in flow mapping"); break; }
        advance(p);

        YMLValue *val = parse_node(p, 0);
        if (p->ok || !val) { free(key); break; }
        hm_set(hm, key, *val);
        free(key); free(val);
    }
    if (cur(p)->type == TK_FLOW_MAP_END) advance(p);
    return v;
}
...
static void apply_merge(_hm *dst, const YMLValue *src) {
    ...
    if (src->type == YML_OBJECT) {
        ...
        size_t idx = 0; const char *key; YMLValue *val;
        while (hm_next(src_hm, &idx, &key, &val)) {
            if (!hm_get(dst, key)) { /* не перезаписывать */
                YMLValue *cp = yml_deep_copy(val);
                if (cp) { hm_set(dst, key, *cp); free(cp); }
            }
        }
    } else if (src->type == YML_ARRAY) {
        /* << может быть массивом маппингов */
        size_t n = da_len(src->value.array);
        for (size_t i = 0; i < n; i++) apply_merge(dst, &src->value.array[i]);
    }
}
...
static YMLValue *parse_block_mapping(Parser *p, int indent) {
    ...
    for (;;) {
        ...
        if (!explicit_key && !implicit_key) break;

        if (explicit_key) advance(p); /* skip ? */

        /* ключ */
        if (cur(p)->type != TK_SCALAR) { parse_error(p, "expected scalar key"); break; }
        char *key = token_strdup(cur(p));
        if (!key) { parse_error(p, "OOM"); break; }
        advance(p);
        ...
        if (cur(p)->type != TK_MAP_VAL) {
            /* нет значения — null */
            YMLValue null_v = { .type = YML_NULL };
            if (strcmp(key,"<<")==0) { free(key); continue; }
...
static YMLValue *parse_node(Parser *p, int min_indent) {
    ...
    /* необязательный тег */
    const char *tag = NULL; size_t tag_len = 0;
    if (cur(p)->type == TK_TAG) {
        tag = cur(p)->value; tag_len = cur(p)->value_len;
        advance(p);
    }

    /* необязательный якорь */
    const char *anchor_name = NULL; size_t anchor_len = 0;
    if (cur(p)->type == TK_ANCHOR) {
        anchor_name = cur(p)->value; anchor_len = cur(p)->value_len;
        advance(p);
    }
    ...
    if (t->type == TK_ALIAS) {
        ...
        if (!orig) { parse_error(p, "unknown alias"); return NULL; }
        advance(p);
    ...
    if (tag) apply_tag(v, tag, tag_len);

    if (anchor_name) anchor_add(p, anchor_name, anchor_len, v);
...
YMLValue **_YMLParseStream(const char *yml_str, struct _YMLOptionals optionals) {
    ...
    Parser p = { .tokens=tokens, .pos=0,
                 .anchors=da_new(_anchor_entry, 8), .ok=0 };

    YMLValue **docs = da_new(YMLValue*, 4);

    while (cur(&p)->type != TK_EOF) {
        if (cur(&p)->type == TK_DOC_START) advance(&p);
        if (cur(&p)->type == TK_EOF) break;
        YMLValue *doc = parse_node(&p, 0);
        if (p.ok) { set_error(p.ok, p.error); break; }
        da_push(docs, doc);
        if (cur(&p)->type == TK_DOC_END) advance(&p);
    }
