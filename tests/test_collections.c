...
int main(void) {
    ...
    root = YMLParse("a: ~\nb:\nc: 1\n", .ok=&ok);
    CHECK(ok==0, "parse");
    v = YMLMapGet(root->value.object, "b", .ok=&ok); CHECK(ok==0 && v && v->type==YML_NULL, "empty value = null");
    YMLDestroy(root);
    ...
    YMLValue *x = YMLMapGet(v->value.object, "x", .ok=&ok); CHECK(ok==0 && x && x->value.integer==1, "x=1");
    YMLValue *y = YMLMapGet(v->value.object, "y", .ok=&ok); CHECK(ok==0 && y && y->value.integer==2, "y=2");
    YMLDestroy(root);

    SECTION("flow mapping at root");
    root = YMLParse("{foo: bar, n: 42}\n", .ok=&ok);
    CHECK(ok==0 && root && root->type==YML_OBJECT, "parse");
    v = YMLMapGet(root->value.object, "foo", .ok=&ok); CHECK_CSTR(v ? v->value.string : NULL, "bar", "foo=bar");
    v = YMLMapGet(root->value.object, "n", .ok=&ok);   CHECK(ok==0 && v && v->value.integer==42, "n=42");
    YMLDestroy(root);
